...
```{code-cell} ipython3
f = pd.read_csv('data/marathon.csv')
f
```
...
```{code-cell} ipython3
# à vous
df_london_1981 = f[(f['city'] == 'LONDON') & (f['year'] == 1981)]
df_london_1981
```
...
```{code-cell} ipython3
# à vous
s1_paris_2000 = f['year'][(f['year'] >= 2000) & (f['city'] == 'PARIS')]
s1_paris_2000
...
```{code-cell} ipython3
# à vous
df_paris_2000_ys = f.loc[(f['city'] == 'PARIS') & (f['year'] >= 2000), ['year', 'seconds']]
df_paris_2000_ys
```
...
```{code-cell} ipython3
# votre code
seconds_average = (f['seconds'].sum())/(f['seconds'].count())
```
...
```{code-cell} ipython3
# à vous
count_by_year = f['year'].value_counts()
count_by_year
```
...
```{code-cell} ipython3
f.groupby('city')['seconds'].mean()
```

affichez la moyenne des durées des marathons par ville triée en ordre croissant

```{code-cell} ipython3
f.groupby('city')['seconds'].mean().sort_values(ascending=False)
```
...
```{code-cell} ipython3
# à vous
f['duration'] = pd.to_timedelta(f['duration'])
f
```
...
f.duration.dtype == 'timedelta64[ns]'
```
...
```{code-cell} ipython3
f['seconds'] == f['duration'].dt.total_seconds()
```
...
```{code-cell} ipython3
f['hour'] = f['seconds']//3600
f['minute'] = f['seconds']//60 - f['hour']*60
f['second'] = f['seconds'] - f['hour']*3600 - f['minute']*60
f
```
...
(    np.all(f.loc[0, ['hour', 'minute', 'second']] == [2, 6, 29])
 and f.hour.dtype == int
 and f.minute.dtype == int 
 and f.second.dtype == int)
```

supprimer les colonnes 'hour', 'minute', 'second' que vous avez ajoutées:

```{code-cell} ipython3
f = f.drop(columns = ['hour', 'minute', 'second'])
f
```
...
```{code-cell} ipython3
comp = f['duration'].dt.components
f['hour'] = comp['hours']
f['minute'] = comp['minutes']
f['second'] = comp['seconds']
f
```
