...
df['Name'][552]  # organisé en colonnes pour les indices
```
...
```{code-cell} ipython3
doc = pd.read_csv('data/petit-titanic.csv')
doc.head(2) #les données se confondent : les colonnes sont séparées par des ;
            #les colonnes ne sont pas nommées et il n'y a aucune ligne pour le faire 
```
...
```{code-cell} ipython3
doc = pd.read_csv('data/petit-titanic.csv', sep = ';', header = None, names = ['PassengerId', 'Survived', 'Pclass', 'Name', 'Sex', 'Age', 'SibSp',
       'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked'])
doc.head(2)
```
...
```{code-cell} ipython3
doc.columns = ['Identifiant', 'Survécu', 'Pclass',
           'Nom', 'Genre', 'Age', 'SibSp', 'Parch',
           'Ticket', 'Tarif', 'Cabine', 'Embarquement']
doc
```
