...
for pays in by_country:
    dfboucle = pd.DataFrame(by_country[pays])
    dfboucle['country'] = pays
    liste.append(dfboucle)
global_df = pd.concat(liste, axis = 0)
global_df
```
...
```{code-cell} ipython3
df4 = clean_df.loc[['France', 'Italy', 'Germany'], ['deaths', 'confirmed']]
```
...
def extract(liste_pays, liste_mesures, debut, fin):
    df = clean_df.loc[(liste_pays, slice(debut, fin)), liste_mesures]
    return(df.unstack(level = 'country'))
```

en utilisant cette fonction, plottez sur un même graphique les données de deux pays

```{code-cell} ipython3
extract(['France', 'US'], ['deaths', 'confirmed'], '2021-01-01', '2022-01-01').plot()
```

## proposez des analyses personnelles sur ces données

```{code-cell} ipython3
# évolution des décès dans le monde car on ne se rend pas compte sur les graphes précédents
global_df.groupby('date')['deaths'].sum().plot()
```

```{code-cell} ipython3
# pour la france 
clean_df.loc['France', 'deaths'].plot()
```
